...
import { ObjectKey, PropertyItem, CreateFormState, CreateStatus } from '../types/propertyTypes';
import { slugify, validateInternalName, ensureUniqueName } from '../utils/propertyUtils';
...
export const AddPropertyModal: React.FC<AddPropertyModalProps> = ({
  ...
  const [createForm, setCreateForm] = useState<CreateFormState>({ label: "", name: "" });
  const [createStatus, setCreateStatus] = useState<CreateStatus>("idle");
  const [createError, setCreateError] = useState<string>("");
  const [isInternalNameEditable, setIsInternalNameEditable] = useState(false);
  const [nameValidationError, setNameValidationError] = useState("");
  const [selectedGroup, setSelectedGroup] = useState<string>("custom_properties");
  ...
  useEffect(() => {
    ...
    const uniqueName = ensureUniqueName(base, targetObject, propPool, defaultMapModal);

    if (!isInternalNameEditable) {
      setCreateForm((f) => ({ ...f, name: uniqueName }));
      setNameValidationError(validateInternalName(uniqueName, targetObject, propPool, defaultMapModal));
    }
  }, [createForm.label, createChecked, targetObject, propPool, defaultMapModal, isInternalNameEditable]);
  ...
  const createTargetPropertyIfNeeded = async (): Promise<PropertyItem | null> => {
    ...
    const name = ensureUniqueName(baseName, addObject, propPool, defaultMapModal);
    ...
    console.log('🏗️ [CREATE_PROPERTY] createForm.name (ignored):', createForm.name);
    ...
    const newProp: PropertyItem = { name, label, type: srcType, fieldType: srcFieldType };

    console.log('🏗️ [CREATE_PROPERTY] Final newProp object:', newProp);

    // Create property in HubSpot via API
    try {
      console.log('🏗️ [CREATE_PROPERTY] Creating property in HubSpot via API...');
      ...
      if (!profile.hubspot_access_token_a) {
        throw new Error('No HubSpot access token found. Please reconnect your HubSpot account.');
      }
      ...
      if (addObject === "deals") {
        ...
        if (!dealNameValidation) {
          throw new Error('Deal property names must start with a letter and contain only lowercase letters, numbers, and underscores.');
        }
        
        // Deal property names must be unique and not conflict with existing HubSpot deal properties
        if (newProp.name.length > 50) {
          throw new Error('Deal property names must be 50 characters or less.');
        }
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(`Failed to create property in HubSpot (HTTP ${response.status}): ${errorText.substring(0, 100)}`);
        }
      }
      ...
      if (!result.ok) {
        throw new Error(result.error || 'Failed to create property in HubSpot');
      }

      console.log('✅ [CREATE_PROPERTY] Property successfully created in HubSpot');

    } catch (error) {
      console.error('❌ [CREATE_PROPERTY] Error creating property in HubSpot:', error);
      setCreateError(error instanceof Error ? error.message : 'Failed to create property in HubSpot');
      setCreateStatus("error");
      return null;
    }

    // persist in prop pool for this object
    const existingList = (propPool[addObject] ?? defaultMapModal[addObject]) as PropertyItem[];
    const updatedList = existingList.some((p) => p.name === newProp.name) ? existingList : [...existingList, newProp];

    console.log('🏗️ [CREATE_PROPERTY] Adding to propPool:');
    console.log('🏗️ [CREATE_PROPERTY] newProp before adding:', newProp);
    console.log('🏗️ [CREATE_PROPERTY] newProp.name should be internal name:', newProp.name);
    console.log('🏗️ [CREATE_PROPERTY] newProp.label should be display label:', newProp.label);
    console.log('🏗️ [CREATE_PROPERTY] existingList length:', existingList.length);
    console.log('🏗️ [CREATE_PROPERTY] updatedList length:', updatedList.length);
    console.log('🏗️ [CREATE_PROPERTY] Last item in updatedList:', updatedList[updatedList.length - 1]);
    ...
    try {
      ...
      if (!prevUD.includes(newProp.name)) {
        ...
        console.log('🏗️ [CREATE_PROPERTY] Added to userDefinedProps localStorage:', newProp.name);
        console.log('🏗️ [CREATE_PROPERTY] Updated userDefinedProps list:', prevUD);
      }
    } catch (error) {
      console.error('❌ [CREATE_PROPERTY] Error updating localStorage:', error);
    }
    ...
    try {
      ...
      if (!data.some((p: any) => p.name === newProp.name)) {
        const merged = [...data, { name: newProp.name, label: newProp.label, type: srcType, fieldType: srcFieldType }];
        localStorage.setItem(cacheKey, JSON.stringify({ data: merged, timestamp: Date.now() }));
        console.log('🏗️ [CREATE_PROPERTY] Added to customProperties cache:', newProp.name);
      }
    } catch (error) {
    ...
    console.log('✅ [CREATE_PROPERTY] Property created successfully:', newProp);
    return newProp;
  };

  const handleInternalNameChange = (value: string) => {
    const newName = value.toLowerCase().replace(/[^a-z0-9_]/g, "_");
    const error = validateInternalName(newName, targetObject, propPool, defaultMapModal);
  ...
  const handleSave = async () => {
    try {
      ...
      console.log('🔄 [MODAL_SAVE] selectedTargetName before:', selectedTargetName);
      ...
      if (!createChecked && selectedSource && selectedTargetName) {
        if (selectedSource.name === selectedTargetName) {
          alert('❌ Error: Source and target properties cannot be the same. Please select a different target property.');
          return;
        }
      }
      ...
      if (createChecked && selectedSource && createForm.label.trim()) {
        console.log('🔄 [MODAL_SAVE] Creating custom property (checkbox checked)...');
        ...
        console.log('✅ [MODAL_SAVE] Custom property created in HubSpot:', createdProperty);
        console.log('✅ [MODAL_SAVE] Final target name set to:', finalTargetName);
      } else {
        console.log('🔄 [MODAL_SAVE] Not creating custom property (checkbox unchecked), using existing property');
      ...
      console.log('🔄 [MODAL_SAVE] Calling parent onSave with createdProperty:', createdProperty);
  ...
  console.log('🔍 [MODAL_RENDER] Modal is rendering, isOpen:', isOpen);

  return (
         <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-200">Add Custom Property Mapping</h2>
          <button 
            onClick={() => {
        ...
        <div className="space-y-6">
          {/* Source object */}
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-300">Select Source Object</label>
            <select
          ...
          {/* Source property */}
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-300">Select Source Property</label>
            <select
              className="w-full rounded-lg border border-gray-600 bg-gray-700 text-gray-200 p-3 text-sm"
            ...
            {selectedSource && (
              <p className="text-xs text-gray-400 mt-2">
                Source type: <b>{selectedSource.type || "-"}</b> • fieldType: <b>{selectedSource.fieldType || "-"}</b>
              </p>
            )}
          </div>
          ...
          {/* Target property */}
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-300">Select Target Property</label>
            <select
            ...
            {selectedSource && !createChecked && filteredTargetList.length === 0 && (
              <p className="text-xs text-amber-400 mt-2">
                No property matches type <b>{selectedSource.type}</b> & fieldType <b>{selectedSource.fieldType}</b>.
              </p>
            )}
            
            {selectedSource && selectedTargetName && selectedSource.name === selectedTargetName && (
              <p className="text-xs text-red-400 mt-2">
                ⚠️ Warning: Source and target properties are the same. This mapping will be ignored.
              </p>
            )}
          </div>
          ...
          <div>
            <label className="inline-flex items-center gap-2">
              ...
              />
              <span className="text-sm text-gray-300 select-none">Create custom property</span>
            </label>
            
            {!profile?.hubspot_access_token_a && (
              <div className="mt-2 text-xs text-yellow-400">
                ⚠️ HubSpot connection required. Please reconnect your HubSpot account in Step 1.
              </div>
            )}
            ...
            {createChecked && (
              <div className="mt-3 grid grid-cols-1 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Property Group</label>
                  <select
                    className="w-full rounded-lg border border-gray-600 bg-gray-700 text-gray-200 p-3 text-sm"
                    value={selectedGroup}
                  ...
                  >
                    ...
                    <option value="companyinformation">Company Information</option>
                    <option value="ticketinformation">Ticket Information</option>
                  </select>
                  <p className="text-xs text-gray-400 mt-1">
                    Choose the group where this property will be created in HubSpot
                  </p>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-sm font-medium text-gray-300">Internal name</label>
                    <button
                      type="button"
                      className="text-xs text-blue-400 hover:text-blue-300 underline"
                      onClick={() => setIsInternalNameEditable(!isInternalNameEditable)}
                    >
                      {isInternalNameEditable ? 'Use auto-generated' : 'Edit manually'}
                    </button>
                  </div>

                  <input
                    className={`w-full rounded-lg border ${nameValidationError ? 'border-red-500' : 'border-gray-600'
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Will be created with type <b>{selectedSource?.type || "-"}</b> & fieldType <b>{selectedSource?.fieldType || "-"}</b>.
                  </p>

                  <div className="mt-3 flex items-center gap-3">
                    {createStatus === "creating" && (
                      <span className="text-blue-400 text-sm">Creating property in HubSpot...</span>
                    )}
                    {createStatus === "created" && (
                      <span className="text-green-400 text-sm">Created ✓ & added to dropdown</span>
                    )}
                    {createStatus === "error" && (
                      <div className="text-red-400 text-sm">
                        <div className="font-medium">Error creating property:</div>
                        ...
                        {createError.includes('property group') && createError.includes('does not exist') && (
                          <div className="text-xs mt-2 text-yellow-400">
                            💡 Try selecting a different property group from the dropdown
                          </div>
                        )}
                        {createError.includes('Deal property names') && (
                          <div className="text-xs mt-2 text-yellow-400">
                            💡 Deal property names must start with a letter and use only lowercase letters, numbers, and underscores
                          </div>
                        )}
                      </div>
                    )}
            ...
            >
              Cancel
               ...
               onClick={() => {
                 ...
                 console.log('🔍 [MODAL_CREATE] selectedProperty:', selectedProperty);
                 console.log('🔍 [MODAL_CREATE] selectedTargetName:', selectedTargetName);
                 console.log('🔍 [MODAL_CREATE] createChecked:', createChecked);
                 console.log('🔍 [MODAL_CREATE] createForm.label:', createForm.label);
                 console.log('🔍 [MODAL_CREATE] selectedSource:', selectedSource);
                 handleSave();
               }}
                               disabled={!selectedProperty || (!createChecked && !selectedTargetName) || createStatus === "creating" || !profile?.hubspot_access_token_a}
