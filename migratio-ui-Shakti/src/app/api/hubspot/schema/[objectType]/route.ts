...
export async function GET(req: NextRequest, { params }: { params: { objectType: string } }) {
  const objectType = params.objectType;
  const { searchParams } = new URL(req.url);
  const propertyType = searchParams.get("propertyType") || "all";
  const instance = searchParams.get("instance") || "a";

  const auth = req.headers.get("authorization") || ""; // Bearer <token> (profile से)
  const userId = req.headers.get("x-user-id") || "";
  const portalId = req.headers.get("x-portal-id") || "";

  const url = `${BACKEND_URL}/hubspot/schema/${objectType}?propertyType=${propertyType}&instance=${instance}`;
  ...
  const r = await fetch(url, {
    ...
    // no-cache to force fresh on Refresh button
    cache: "no-store",
  });
