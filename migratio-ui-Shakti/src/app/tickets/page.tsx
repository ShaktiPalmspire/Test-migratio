...
export default function TicketsPage() {
  ...
  const TicketsCache = useAppSelector((s) => selectTicketsCache(s, "A"));
  ...
  // UI state
  const [TicketsA, setTicketsA] = React.useState<any[]>([]);
  ...
  const LS_Tickets_KEY = React.useMemo(
    () => lsKey("cached_Tickets_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  const LS_Tickets_TIME_KEY = React.useMemo(
    () => lsKey("cached_Tickets_time_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  ...
  React.useEffect(() => {
    ...
    const hasRedux = !!TicketsCache?.results?.length;
    const hasLS = !!localStorage.getItem(LS_Tickets_KEY);
  ...
  }, [user?.id, profile?.hubspot_access_token_a, LS_Tickets_KEY, TicketsCache, dispatch, backendBase]);
  ...
  const fetchDealPropertyLabels = React.useCallback(async () => {
    ...
    const cacheKey = `schema:Tickets:labels:${profile?.hubspot_portal_id_a}`;
    ...
    try {
      ...
      const urls = [
        `/api/hubspot/schema/Tickets?propertyType=all&userId=${user.id}&instance=a`,
        `/api/hubspot/schema/Tickets?propertyType=custom&userId=${user.id}&instance=a`,
      ];
      ...
      console.warn("[Tickets] property label fetch failed", e);
      setDealLabelMap(base);
    }
  ...
  React.useEffect(() => {
    ...
    if (switched) {
      ...
      setDataSource("none");
      dispatch(clearTicketsCache("A"));
      const oldDataKey = lsKey("cached_Tickets_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
      const oldTimeKey = lsKey("cached_Tickets_time_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
  ...
  const smartRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("[Tickets] smartRefresh failed", e);
      dispatch(setError({ instance: "A", error: "Failed to refresh Tickets. Please try again." }));
    }
  ...
  React.useEffect(() => {
    ...
    try {
      const cachedData = localStorage.getItem(LS_Tickets_KEY);
      const cachedTime = localStorage.getItem(LS_Tickets_TIME_KEY);

      if (cachedData) {
        const parsed = JSON.parse(cachedData);

        setTicketsA(parsed?.results ?? []);
        setNextAfterA(parsed?.paging?.next?.after ?? null);
        setDataSource("cache");

        if (!TicketsCache?.results?.length) {
          const t = cachedTime ? parseInt(cachedTime, 10) : Date.now();
          dispatch(setTicketsCache({ instance: "A", data: parsed, timestamp: t }));
        }
      ...
      console.error("[Tickets] SWR load failed", e);
  ...
  React.useEffect(() => {
    if (TicketsCache && lastFetchTime) {
      try {
        localStorage.setItem(LS_Tickets_KEY, JSON.stringify(TicketsCache));
        localStorage.setItem(LS_Tickets_TIME_KEY, lastFetchTime.toString());
      } catch (e) {
        console.error("[Tickets] save LS failed", e);
      }
    }
  }, [TicketsCache, lastFetchTime, LS_Tickets_KEY, LS_Tickets_TIME_KEY]);
    ...
    async (after?: string | null) => {
      ...
      try {
        ...
        if (isExpired || minutesLeft < 1) {
          ...
          if (!ok) {
            dispatch(setError({ instance: "A", error: "Failed to refresh token. Cannot fetch Tickets." }));
            return;
          }
        ...
        await dispatch(fetchTicketsAsync({ instance: "A", after, userId: user.id, backendBase })).unwrap();
        setDataSource("api");
      } catch (e: any) {
  ...
  React.useEffect(() => {
    if (TicketsCache?.results?.length) {
      setTicketsA(TicketsCache.results);
      setNextAfterA(TicketsCache.paging?.next?.after ?? null);
    }
  }, [TicketsCache]);
  ...
  const rowData = React.useMemo(() => TicketsA.map((c: any) => ({ id: c.id, ...c.properties })), [TicketsA]);

  const columnDefs = React.useMemo((): ColDef[] => {
    if (TicketsA.length === 0) return [];
    const allKeys = Array.from(new Set(TicketsA.flatMap((c) => Object.keys(c.properties || {}))));
    const cols: ColDef[] = [
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex items-center gap-2 p-4">
          ...
          <h2 className="text-2xl font-semibold">Tickets (HubSpot Portal A)</h2>
        </header>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-sm text-gray-600 mb-6">Please connect your HubSpot CRM A to view and manage Tickets.</p>
                <button
            ...
            <>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <p className="text-xs text-green-500 mt-1">{loadingA ? "Loading..." : `${TicketsA.length} Tickets loaded`}</p>

                  {/* 🔐 Admin-only source badge */}
                  {showAdmin && TicketsA.length > 0 && <DataSourceBadge source={dataSource} />}
                      ...
                      {TicketsA.length > 0 ? (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Refreshing..." : "🔄 Refresh Tickets"}
                            </button>
                            <span className="text-xs text-gray-600">📊 {TicketsA.length} Tickets loaded</span>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Loading..." : "📥 Load Tickets"}
                            </button>
                          </div>
                        </div>
                      )}
              ...
              {showAdmin && (
                <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-600">
                  ...
                  <div className="text-left text-xs text-gray-600 dark:text-gray-400 space-y-2">
                    ...
                    <div>
                      <strong>Tickets Cache:</strong>{" "}
                      {TicketsCache ? `✅ ${TicketsCache.results?.length || 0} Tickets` : "❌ None"}
                    </div>
                    <div>
                    ...
                    <div>
                      <strong>Tickets Loaded:</strong> {TicketsA.length}
                    </div>
                  </div>
                </div>
              )}
              ...
              {TicketsA.length > 0 ? (
                <>
                  <div className="text-xs text-gray-500 mb-2">
                    ROW: {TicketsA.length}, COLS: {columnDefs.length}
                  </div>
                  ...
                  <div className="mt-4">
                    {profile?.hubspot_access_token_a && (
                      <button
                        ...
                        onClick={() => fetchTickets(nextAfterA)}
                        disabled={!nextAfterA || loadingA || autoRefreshing}
                      >
                        {loadingA || autoRefreshing ? "Loading..." : nextAfterA ? "Load More" : "All Tickets Loaded"}
                      </button>
                    )}
                  </div>
                </>
