...
export default function ContactsPage() {
  ...
  const ContactsCache = useAppSelector((s) => selectContactsCache(s, "A"));
  ...
  // UI state
  const [ContactsA, setContactsA] = React.useState<any[]>([]);
  ...
  const LS_Contacts_KEY = React.useMemo(
    () => lsKey("cached_Contacts_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  const LS_Contacts_TIME_KEY = React.useMemo(
    () => lsKey("cached_Contacts_time_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  ...
  React.useEffect(() => {
    ...
    const hasRedux = !!ContactsCache?.results?.length;
    const hasLS = !!localStorage.getItem(LS_Contacts_KEY);
  ...
  }, [user?.id, profile?.hubspot_access_token_a, LS_Contacts_KEY, ContactsCache, dispatch, backendBase]);
  ...
  const fetchDealPropertyLabels = React.useCallback(async () => {
    ...
    const cacheKey = `schema:Contacts:labels:${profile?.hubspot_portal_id_a}`;
    ...
    try {
      ...
      const urls = [
        `/api/hubspot/schema/Contacts?propertyType=all&userId=${user.id}&instance=a`,
        `/api/hubspot/schema/Contacts?propertyType=custom&userId=${user.id}&instance=a`,
      ];
      ...
      console.warn("[Contacts] property label fetch failed", e);
      setDealLabelMap(base);
    }
  ...
  React.useEffect(() => {
    ...
    if (switched) {
      ...
      setDataSource("none");
      dispatch(clearContactsCache("A"));
      const oldDataKey = lsKey("cached_Contacts_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
      const oldTimeKey = lsKey("cached_Contacts_time_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
  ...
  const smartRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("[Contacts] smartRefresh failed", e);
      dispatch(setError({ instance: "A", error: "Failed to refresh Contacts. Please try again." }));
    }
  ...
  React.useEffect(() => {
    ...
    try {
      const cachedData = localStorage.getItem(LS_Contacts_KEY);
      const cachedTime = localStorage.getItem(LS_Contacts_TIME_KEY);

      if (cachedData) {
        const parsed = JSON.parse(cachedData);

        setContactsA(parsed?.results ?? []);
        setNextAfterA(parsed?.paging?.next?.after ?? null);
        setDataSource("cache");

        if (!ContactsCache?.results?.length) {
          const t = cachedTime ? parseInt(cachedTime, 10) : Date.now();
          dispatch(setContactsCache({ instance: "A", data: parsed, timestamp: t }));
        }
      ...
      console.error("[Contacts] SWR load failed", e);
    ...
    LS_Contacts_KEY,
    LS_Contacts_TIME_KEY,
    ContactsCache?.results?.length,
    dispatch,
    smartRefresh,
  ]);
  ...
  React.useEffect(() => {
    if (ContactsCache && lastFetchTime) {
      try {
        localStorage.setItem(LS_Contacts_KEY, JSON.stringify(ContactsCache));
        localStorage.setItem(LS_Contacts_TIME_KEY, lastFetchTime.toString());
      } catch (e) {
        console.error("[Contacts] save LS failed", e);
      }
    }
  }, [ContactsCache, lastFetchTime, LS_Contacts_KEY, LS_Contacts_TIME_KEY]);
    ...
    async (after?: string | null) => {
      ...
      try {
        ...
        if (isExpired || minutesLeft < 1) {
          ...
          if (!ok) {
            dispatch(setError({ instance: "A", error: "Failed to refresh token. Cannot fetch Contacts." }));
            return;
          }
        ...
        await dispatch(fetchContactsAsync({ instance: "A", after, userId: user.id, backendBase })).unwrap();
        setDataSource("api");
      } catch (e: any) {
  ...
  React.useEffect(() => {
    if (ContactsCache?.results?.length) {
      setContactsA(ContactsCache.results);
      setNextAfterA(ContactsCache.paging?.next?.after ?? null);
    }
  }, [ContactsCache]);
  ...
  const rowData = React.useMemo(() => ContactsA.map((c: any) => ({ id: c.id, ...c.properties })), [ContactsA]);

  const columnDefs = React.useMemo((): ColDef[] => {
    if (ContactsA.length === 0) return [];
    const allKeys = Array.from(new Set(ContactsA.flatMap((c) => Object.keys(c.properties || {}))));
    const cols: ColDef[] = [
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex items-center gap-2 p-4">
          ...
          <h2 className="text-2xl font-semibold">Contacts (HubSpot Portal A)</h2>
        </header>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-sm text-gray-600 mb-6">Please connect your HubSpot CRM A to view and manage Contacts.</p>
                <button
            ...
            <>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <p className="text-xs text-green-500 mt-1">{loadingA ? "Loading..." : `${ContactsA.length} Contacts loaded`}</p>

                  {/* 🔐 Admin-only source badge */}
                  {showAdmin && ContactsA.length > 0 && <DataSourceBadge source={dataSource} />}
                      ...
                      {ContactsA.length > 0 ? (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Refreshing..." : "🔄 Refresh Contacts"}
                            </button>
                            <span className="text-xs text-gray-600">📊 {ContactsA.length} Contacts loaded</span>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Loading..." : "📥 Load Contacts"}
                            </button>
                          </div>
                        </div>
                      )}
              ...
              {showAdmin && (
                <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-600">
                  ...
                  <div className="text-left text-xs text-gray-600 dark:text-gray-400 space-y-2">
                    ...
                    <div>
                      <strong>Contacts Cache:</strong>{" "}
                      {ContactsCache ? `✅ ${ContactsCache.results?.length || 0} Contacts` : "❌ None"}
                    </div>
                    <div>
                    ...
                    <div>
                      <strong>Contacts Loaded:</strong> {ContactsA.length}
                    </div>
                  </div>
                </div>
              )}
              ...
              {ContactsA.length > 0 ? (
                <>
                  <div className="text-xs text-gray-500 mb-2">
                    ROW: {ContactsA.length}, COLS: {columnDefs.length}
                  </div>
                  ...
                  <div className="mt-4">
                    {profile?.hubspot_access_token_a && (
                      <button
                        ...
                        onClick={() => fetchContacts(nextAfterA)}
                        disabled={!nextAfterA || loadingA || autoRefreshing}
                      >
                        {loadingA || autoRefreshing ? "Loading..." : nextAfterA ? "Load More" : "All Contacts Loaded"}
                      </button>
                    )}
                  </div>
                </>
