...
export default function DealsPage() {
  ...
  const DealsCache = useAppSelector((s) => selectDealsCache(s, "A"));
  ...
  // UI state
  const [DealsA, setDealsA] = React.useState<any[]>([]);
  ...
  const LS_Deals_KEY = React.useMemo(
    () => lsKey("cached_Deals_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  const LS_Deals_TIME_KEY = React.useMemo(
    () => lsKey("cached_Deals_time_a", user?.id ?? null, (profile?.hubspot_portal_id_a as any) ?? null),
    [user?.id, profile?.hubspot_portal_id_a]
  );
  ...
  React.useEffect(() => {
    ...
    const hasRedux = !!DealsCache?.results?.length;
    const hasLS = !!localStorage.getItem(LS_Deals_KEY);
  ...
  }, [user?.id, profile?.hubspot_access_token_a, LS_Deals_KEY, DealsCache, dispatch, backendBase]);
  ...
  const fetchDealPropertyLabels = React.useCallback(async () => {
    ...
    const cacheKey = `schema:Deals:labels:${profile?.hubspot_portal_id_a}`;
    ...
    try {
      ...
      const urls = [
        `/api/hubspot/schema/Deals?propertyType=all&userId=${user.id}&instance=a`,
        `/api/hubspot/schema/Deals?propertyType=custom&userId=${user.id}&instance=a`,
      ];
      ...
      console.warn("[Deals] property label fetch failed", e);
      setDealLabelMap(base);
    }
  ...
  React.useEffect(() => {
    ...
    if (switched) {
      ...
      setDataSource("none");
      dispatch(clearDealsCache("A"));
      const oldDataKey = lsKey("cached_Deals_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
      const oldTimeKey = lsKey("cached_Deals_time_a", prevKeyRef.current.uid, prevKeyRef.current.portal);
  ...
  const smartRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("[Deals] smartRefresh failed", e);
      dispatch(setError({ instance: "A", error: "Failed to refresh Deals. Please try again." }));
    }
  ...
  React.useEffect(() => {
    ...
    try {
      const cachedData = localStorage.getItem(LS_Deals_KEY);
      const cachedTime = localStorage.getItem(LS_Deals_TIME_KEY);

      if (cachedData) {
        const parsed = JSON.parse(cachedData);

        setDealsA(parsed?.results ?? []);
        setNextAfterA(parsed?.paging?.next?.after ?? null);
        setDataSource("cache");

        if (!DealsCache?.results?.length) {
          const t = cachedTime ? parseInt(cachedTime, 10) : Date.now();
          dispatch(setDealsCache({ instance: "A", data: parsed, timestamp: t }));
        }
      ...
      console.error("[Deals] SWR load failed", e);
  ...
  React.useEffect(() => {
    if (DealsCache && lastFetchTime) {
      try {
        localStorage.setItem(LS_Deals_KEY, JSON.stringify(DealsCache));
        localStorage.setItem(LS_Deals_TIME_KEY, lastFetchTime.toString());
      } catch (e) {
        console.error("[Deals] save LS failed", e);
      }
    }
  }, [DealsCache, lastFetchTime, LS_Deals_KEY, LS_Deals_TIME_KEY]);
    ...
    async (after?: string | null) => {
      ...
      try {
        ...
        if (isExpired || minutesLeft < 1) {
          ...
          if (!ok) {
            dispatch(setError({ instance: "A", error: "Failed to refresh token. Cannot fetch Deals." }));
            return;
          }
        ...
        await dispatch(fetchDealsAsync({ instance: "A", after, userId: user.id, backendBase })).unwrap();
        setDataSource("api");
      } catch (e: any) {
  ...
  React.useEffect(() => {
    if (DealsCache?.results?.length) {
      setDealsA(DealsCache.results);
      setNextAfterA(DealsCache.paging?.next?.after ?? null);
    }
  }, [DealsCache]);
  ...
  const rowData = React.useMemo(() => DealsA.map((c: any) => ({ id: c.id, ...c.properties })), [DealsA]);

  const columnDefs = React.useMemo((): ColDef[] => {
    if (DealsA.length === 0) return [];
    const allKeys = Array.from(new Set(DealsA.flatMap((c) => Object.keys(c.properties || {}))));
    const cols: ColDef[] = [
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex items-center gap-2 p-4">
          ...
          <h2 className="text-2xl font-semibold">Deals (HubSpot Portal A)</h2>
        </header>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-sm text-gray-600 mb-6">Please connect your HubSpot CRM A to view and manage Deals.</p>
                <button
            ...
            <>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <p className="text-xs text-green-500 mt-1">{loadingA ? "Loading..." : `${DealsA.length} Deals loaded`}</p>

                  {/* 🔐 Admin-only source badge */}
                  {showAdmin && DealsA.length > 0 && <DataSourceBadge source={dataSource} />}
                      ...
                      {DealsA.length > 0 ? (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Refreshing..." : "🔄 Refresh Deals"}
                            </button>
                            <span className="text-xs text-gray-600">📊 {DealsA.length} Deals loaded</span>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {loadingA || autoRefreshing ? "Loading..." : "📥 Load Deals"}
                            </button>
                          </div>
                        </div>
                      )}
              ...
              {showAdmin && (
                <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-600">
                  ...
                  <div className="text-left text-xs text-gray-600 dark:text-gray-400 space-y-2">
                    ...
                    <div>
                      <strong>Deals Cache:</strong>{" "}
                      {DealsCache ? `✅ ${DealsCache.results?.length || 0} Deals` : "❌ None"}
                    </div>
                    <div>
                    ...
                    <div>
                      <strong>Deals Loaded:</strong> {DealsA.length}
                    </div>
                  </div>
                </div>
              )}
              ...
              {DealsA.length > 0 ? (
                <>
                  <div className="text-xs text-gray-500 mb-2">
                    ROW: {DealsA.length}, COLS: {columnDefs.length}
                  </div>
                  ...
                  <div className="mt-4">
                    {profile?.hubspot_access_token_a && (
                      <button
                        ...
                        onClick={() => fetchDeals(nextAfterA)}
                        disabled={!nextAfterA || loadingA || autoRefreshing}
                      >
                        {loadingA || autoRefreshing ? "Loading..." : nextAfterA ? "Load More" : "All Deals Loaded"}
                      </button>
                    )}
                  </div>
                </>
