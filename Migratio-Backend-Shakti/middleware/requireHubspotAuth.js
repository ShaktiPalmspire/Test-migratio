...
// ✅ NEW: detect whether we can safely validate against Supabase
...
module.exports = function requireHubspotAuth(req, res, next) {
  ...
  // If no session token, try Authorization: Bearer <token>
  if (!token) {
    const authHeader = req.headers.authorization;
    if (authHeader && authHeader.startsWith('Bearer ')) {
      const bearerToken = authHeader.substring(7);

      // ✅ NEW: if Supabase is not enabled (dev), trust the bearer token and continue
      if (!supabaseEnabled) {
        req.hubspot = { accessToken: bearerToken };
        return next();
      }

      // Else: validate token via Supabase (existing behavior)
      return validateBearerToken(bearerToken, req, res, next);
    }
  }

  if (!token) {
    // Keep existing behavior for missing token
    return res.status(401).json({ success: false, message: 'No HubSpot token found' });
  }
...
async function validateBearerToken(bearerToken, req, res, next) {
  try {
    if (!supabaseEnabled) {
      // Should never hit due to early return above, but guard anyway
      req.hubspot = { accessToken: bearerToken };
      return next();
    }

    // === existing Supabase lookup logic (unchanged) ===
    const { data: profiles, error } = await supabase
      .from('profiles')
      .select('id, hubspot_access_token_a, hubspot_access_token_b, hubspot_portal_id_a, hubspot_portal_id_b')
      .or(`hubspot_access_token_a.eq.${bearerToken},hubspot_access_token_b.eq.${bearerToken}`);
